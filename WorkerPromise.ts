...
class WorkerPromise {
  ...
  static resolve (value) {
    return value && value.constructor === WorkerPromise
      ? value
      : new WorkerPromise((resolve) => {
        resolve(value);
      });
  }

  static reject (reason) {
    return new WorkerPromise((_, reject) => {
      reject(reason);
    });
  }

  static all (iterable) {
    return new WorkerPromise((resolve, reject) => {
